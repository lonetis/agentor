...
## Reconcile Robustness (misconfig safety)

Because a port mapping (always) and the first domain mapping/dashboard (which binds host 80/443) trigger a **Traefik container recreate**, a mapping whose host port can't be bound — already taken by another container or a host process, or 80/443 while domain routing is active — would otherwise leave the running Traefik removed and a new one unable to start. In production the dashboard is served *through* Traefik, so this used to lock the operator out with no way to revert the bad mapping. The reconcile path is therefore layered to never let a user misconfiguration wedge Traefik:

1. **Reserved ports (`assertPortAcceptable`)** — the port-mapping create handlers reject external **80/443** with **409** whenever the web entrypoints are active (`needsWebEntrypoints()` — any domain mapping or a configured dashboard subdomain). These are structurally Traefik's own, so a `pm-80`/`pm-443` entrypoint could never coexist with them.
2. **Pre-flight host-port check** — before tearing down the running Traefik, the reconcile computes the **new** host ports (those the running container does not already bind) and checks them against the published ports of all other Docker containers. A clash throws **before** any destructive action, so the working Traefik (and the dashboard) is never touched. Caught by the create handler → **409**, mapping rolled back out of the store.
3. **Transactional create + rollback (`createOrRollback`)** — `TraefikManager` keeps a `lastGoodMappings` snapshot (the mappings that last produced a *running* container). If `createTraefik()` fails to start despite the pre-flight (e.g. a non-Docker host process owns the port, or a TOCTOU race), the manager rebuilds Traefik from the last-good snapshot so the dashboard returns, then re-throws. The half-created container is force-removed before retry.
4. **Strict reconcile + store rollback (`reconcileStrict`)** — mapping-create handlers call `reconcileStrict()`, which *rejects* (unlike the fire-and-forget `reconcile()` used by lifecycle callers) when its apply failed. On rejection the handler deletes the just-persisted mapping and returns **409** — so a mapping that can't be applied never survives in the store to re-break Traefik on the next reconcile. The SSH/auto-port-mapping allocator additionally retries the next free port when its candidate can't bind.
5. **Startup self-healing (`startOrSelfHeal`)** — if a previously-persisted mapping slipped through (e.g. a port freed then re-taken), at startup a found-but-stopped Traefik that fails to start is recreated with a **reduced** config: port mappings whose host port is currently occupied are dropped (keeping the rest + domain routing + dashboard) so the dashboard always comes back. The dropped mapping is left in the store, unrouted, for the operator to inspect and remove.

Net effect: a bad port/domain mapping is rejected with a 409 (usually with **zero** disruption via layers 1–2; at worst a few-second blip via layer 3), and the dashboard is never left unreachable. The fire-and-forget `reconcile()` used by rebuild/unarchive/import/startup benefits from layers 3 and 5 automatically.
...
- `TraefikManager` (`traefik-manager.ts`): Manages the `agentor-traefik` container lifecycle. On mapping changes, writes a Traefik file provider config (`traefik-config.yml`), then ensures the Traefik container exists. Uses `providers.file.watch=true` so config changes are picked up without container restart. Serialized via promise queue. Routes use per-mapping `baseDomain` + `getTlsConfig()` for per-domain cert resolver selection. Dashboard uses `dashboardBaseDomain` from config. The build/create path is parameterized by an explicit `Mappings` snapshot (`{ domain, port }`) — `buildCmd(m)` constructs Traefik CLI args with per-challenge-type resolvers, `buildEnv()` collects DNS provider env vars, `hasContainerConfigDrift(info, m)` detects when running container config diverges from expected — so a reconcile can be replayed against a previous snapshot for rollback/self-heal without mutating the stores. Two reconcile entry points share one queue: fire-and-forget `reconcile()` (logs + self-recovers) and `reconcileStrict()` (rejects so a create handler can roll back the offending mapping). See **Reconcile Robustness** above for the layered misconfig handling (reserved ports, pre-flight, transactional rollback, startup self-heal).
...
- Container recreated on Cmd/Env/PortBindings drift — e.g. adding a DNS provider, adding or removing a port mapping (entrypoint list changes), or switching a port mapping between `localhost`/`external`. A recreate is **pre-flighted**: the new host ports are checked for conflicts before the running container is removed, and a failed (re)create rolls Traefik back to its last-good config so the dashboard is never left down (see **Reconcile Robustness**)
