...
defineRouteMeta({
  openAPI: {
    ...
    parameters: [
      ...
      { name: 'since', in: 'query', schema: { type: 'string' }, description: 'ISO 8601 timestamp — entries with timestamp >= since (inclusive)' },
      { name: 'until', in: 'query', schema: { type: 'string' }, description: 'ISO 8601 timestamp — entries with timestamp < until (exclusive). Pass the oldest entry on the current page to fetch the next older page.' },
